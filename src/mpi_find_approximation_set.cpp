...
void streaming(
    ...
    if (appData.worldRank == 0) {
        ...
        // Used to cancel isends after reciever finds a solution.
        std::atomic_bool foundSolution = false;

        std::unique_ptr<MutableSubset> subset(new StreamingSubset(data, std::floor(appData.outputSetSize * appData.alpha), timers), foundSolution);
        std::unique_ptr<SubsetCalculator> calculator(MpiOrchestrator::getCalculator(appData));
