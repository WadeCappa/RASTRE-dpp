...
class MpiOrchestrator : public Orchestrator {
    ...
    static std::unique_ptr<BucketTitrator> buildTitrator(const AppData &appData, const unsigned int threads, float firstDeltaZero = 0.0) {
        if (appData.distributedAlgorithm == 1) {
            return SieveStreamingBucketTitrator::create(threads, appData.distributedEpsilon, appData.outputSetSize, firstDeltaZero);
        }
        else if (appData.distributedAlgorithm == 2) {
            return ThreeSieveBucketTitrator::create(appData.distributedEpsilon, appData.threeSieveT, appData.outputSetSize, firstDeltaZero);
        } else {
